...
function VHaMQuiz({ onQuizFeedback, onReturnToMain }) {
    ...
    // Get 10 random questions from the quiz data
    ...
    const handleSendFeedbackManually = async () => {
        ...
        const summaryText =
            `I completed the UK Vehicle and Maintenance Quiz. My score was ${score} out of ${questions.length}.\n\n` +
            (wrongSummary ? `Mistakes:\n\n${wrongSummary}` : "I answered all questions correctly.");

        // Define a strict feedback prompt
...
${chunk}
    `;
        ...
        // If no mistakes exist, simply set minimal feedback and redirect if needed.
        if (!wrongSummary) {
