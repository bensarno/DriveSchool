...
function SaLRQ({ onQuizFeedback, onReturnToMain }) {
    ...
    // Handle the manual feedback button click
    const handleSendFeedbackManually = async () => {
        ...
        const summaryText =
            `I completed the UK Safety and Legal Requirements Quiz. My score was ${score} out of ${questions.length}.\n\n` +
            (wrongSummary ? `Mistakes:\n\n${wrongSummary}` : "I answered all questions correctly.");

        // Define the feedback prompt. Note the strict instructions.
...
${chunk}
    `;
        ...
        // If there are no wrong answers, set minimal feedback and redirect
        if (!wrongSummary) {
        ...
        // Redirect the user back if a redirection function is provided
        if (onReturnToMain) {
            onReturnToMain();
        }
    };
    ...
    if (loading)
        return <div className="page3-container">Loading questions...</div>;
    ...
    return (
        <div className="page3-container">
            <h1 className="page3-header">UK Safety and Legal Requirements Quiz</h1>
            {!isQuizFinished ? (
                ...
                <div className="quiz-results">
                    ...
                    <p>
                        Percentage: {(getScore() / questions.length * 100).toFixed(1)}%
                    </p>
                    <h3>Feedback:</h3>
